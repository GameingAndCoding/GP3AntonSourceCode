...
void ATeam12GameMode::Tick(float deltaTime)
{
	...
	// If not paused and entered the start game trigger start timer
	if (!PauseTimer && StartGame) 
	{
		TotalTimeElapsed += deltaTime;

		//make the timer actually time dependant and be consistent with all framerates
...
//return minutes seconds and milliseconds for UI
