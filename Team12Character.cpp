...
void ATeam12Character::BeginPlay()
{
	...
	if(Follower)
	{
		...
		//crete a basic actor for encounters to not end when attacking enemies
		if(followerInstance)
		{
...
//start an attack , if held charges up 
...
// Do attack when releasing attack
...
void ATeam12Character::Move(const FInputActionValue& Value)
{
	...
	//check if only moving forward and not to sides for bunnyhopping
...
void ATeam12Character::GroundCheck()
{
	...
	//double jump and bunny hop stuff
	if (isGrounded) 
	{
		DidDoubleJump = false;
		canBunnyHop= true;
 ...
 void ATeam12Character::DashLineTrace()
{
	if(CurrentDashCharges>0)
	{
		...
		//if hit something check if it can be damaged.
		if (ActorHit) {
			...
			if(EnemyHit!= NULL)
			{
				//if its an enemy keep dashing forward
				DashPierceEnemy(Hit.Location,LineTraceEnd , ActorHit);
			}else if (egg!= NULL)
			{
				//if its an egg keep dashing forward
				DashPierceEnemy(Hit.Location,LineTraceEnd , ActorHit);
			}else if (projectile!= NULL)
			{
				//if its a projectile keep dashing forward
				DashPierceEnemy(Hit.Location,LineTraceEnd , ActorHit);
 ...
 //move to previously selected position
...
//if dashed into an enemy egg or projectile dash again automatically
...
//Method for reseting dash when entering dash resets in game
...
// deal damage and disable their collision of enemies when dashing through them
...
// check if is walking and grounded for sfx
